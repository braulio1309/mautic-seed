...
use Mautic\ChannelBundle\Form\Type\ProductImportFieldType;
use Mautic\ChannelBundle\Form\Type\ProductImportType;
...
class ImportVariantController extends FormController
{
            ...
            return $this->postActionRedirect(
                $this->getPostActionRedirectArguments(
                    [
                        ...
                        'contentTemplate' => 'MauticChannelBundle:Variant:variant_list.html.php',
                        'passthroughVars' => [
    ...
    public function newAction($objectId = 0, $ignorePost = false)
    {
        ...
        if (self::STEP_UPLOAD_CSV === $step || self::STEP_MATCH_FIELDS === $step) {
            $contentTemplate = 'MauticChannelBundle:Import:new.html.php';
            $viewParameters  = [
                'form'       => $form->createView(),
                'objectName' => $initEvent->objectName,
            ];
        } else {
            $contentTemplate = 'MauticChannelBundle:Import:progress.html.php';
            $viewParameters  = [
                'progress'         => $progress,
                'import'           => $import,
                'complete'         => $complete,
